...
EasySorting 是一个面向电赛/硬件打样场景的元器件库存管理工具，支持基于立创商城的 BOM 匹配、订单入库、库存维护和采购单导出。

## 功能概览

- 元件入库：导入立创商城 BOM 报价单，并可选附带订单详情，自动计算可入库数量，支持手动修正数量与元件信息。
...
1. 上传 BOM 报价单 Excel（必填）。
2. 可选上传订单详情 Excel；若未上传，则按报价单购买数量计算入库。
...
1. 在 BOM 页面进入“我的BOM”。
2. 下载有关联订单的 BOM 报价单。
3. 如需按订单差量入库，再进入订单页导出订单详情 Excel。
